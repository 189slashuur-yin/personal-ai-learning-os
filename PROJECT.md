...
截至 2026-07-03，Sprint1–Sprint11、Epic A、Epic B 与 Epic C Search 2.0 已完成；Epic D 的 D0 Architecture Pack v1 已冻结，Task 运行时尚未实现。项目已经从单次 TXT 导入流程演进为可在浏览器中运行的个人知识工作区，并增加面向 ChatGPT、Claude、DeepSeek、Gemini 与普通文本的 Clipboard Import 预处理、结构化本地搜索，以及可选的本地 Ollama Analyzer。

当前重点是验证产品工作流和边界，而不是接入生产基础设施。Demo Provider 默认启用且不访问网络；Ollama 默认关闭，只在用户配置、启用并选择后访问本地 HTTP 服务。云 Provider、数据库和多用户能力均未实现。

Epic D 将在 Knowledge 旁增加独立 Task Domain，解决“接下来做什么”，而不把行动状态塞进 KnowledgeCard。第一阶段只做 Task；Activity 仅为 planned / not immediate，Agent、Calendar 与 RAG 不在范围。AI 最多建议 Task，不能直接创建、完成或删除 Task。设计基线见 [Epic D Design](./docs/design/Epic-D-Design.md)。
...
- Dashboard、本地统计，以及跨 Conversation、Proposal、Knowledge、Tag、Workspace 的关键词与结构化过滤搜索。
