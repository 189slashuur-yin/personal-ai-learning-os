...
截至 2026-07-05，运行时进入 v1.0 alpha draft — Conversation / Round Implementation。Phase0 Architecture Freeze 已完成，Phase1 Epic A–L 已实现 Round 领域、显式兼容迁移、Preview-first Import、Round-first Conversation、Round provenance 与 Search 集成。

- Current Version：v1.0 alpha draft
- Current Focus：Phase1 Epic A–L implementation complete；manual QA pending
- Next Recommended Phase：执行 Phase1 Manual QA；停止在 L，不进入 M–Z/AA
