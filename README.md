...
- Current Epic：Epic D / D3 Source-linked Task
...
Release v0.6 处于 Phase2，Epic A、Epic B、Epic C Search 2.0 与 Epic D D0–D3 已完成。Conversation 与 Task 可归属单层 Workspace；Knowledge、Conversation 和选中 Messages 可显式创建保留 SourceRef 快照的 Task。Today 与 Tasks 会展示来源及删除后的降级状态。数据仍保存在 LocalStorage 中，不需要数据库。
...
全局 Search 当前仍只支持 Conversation、Proposal、Knowledge、Tag 与 Workspace，不索引 Task。
