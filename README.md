...
- Current Focus：v1.0 Phase0 Architecture Freeze candidate（等待人工批准；v0.9 人工 QA 仍待执行）
...
v1.0 Phase0 已完成文档级架构冻结候选：Conversation 被定义为一个逻辑对话线程，Round 是其稳定子实体而非新 Aggregate Root，Proposal/Knowledge 保持独立生命周期，Search 与 Import 使用明确读模型/Parser 边界。该候选仍等待人工批准；当前没有 Round 运行时实现或数据迁移。详见 [Architecture Freeze Report](./docs/reviews/Architecture-Freeze-v1.0-Phase0.md)。
