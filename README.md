...
- Current Phase：v1.0 Phase1 — Conversation / Round Implementation
- Current Focus：Epic A–L 已实现并通过自动门禁；人工 QA 待执行
- Next Recommended Phase：执行 `docs/QA_CHECKLIST.md` 的 v1.0 Phase1 手工验收；不要进入 M–Z/AA
...
| Import | ✅ alpha | Paste/TXT 的版本化 Parser、Parser/Round Preview 与确认写入；JSON 仍为占位。 |
| Tag | ✅ | Tag 管理、关联与筛选。 |
...
v1.0 Phase0 已批准并进入 Phase1。当前 alpha 已实现 Round Entity/Contract/BrowserStorage/Service、显式 Message→Round 预检生成、版本化 Parser/Preview/Confirm、Round-first Conversation、Round 来源 Proposal/Knowledge 与 Round Search。迁移不会自动运行，也不会删除或改写旧 Message；详细边界见 [Migration Report](./docs/migrations/Message-to-Round-v1.0-Phase1.md)。
...
备份包含 `docs/`、README、PROJECT、ARCHITECTURE、ROADMAP、HANDOFF、CHANGELOG，以及存在时的项目内 `data/`。脚本不会读取项目外文件；自定义路径只作为输出目标。浏览器 LocalStorage 和外部 Asset 路径指向的文件不会被脚本自动导出，当前仍需单独保留浏览器数据与原始文件。
