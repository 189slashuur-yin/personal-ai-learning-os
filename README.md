...
- Current Epic：Epic D Completed / v0.7 Daily Learning Workflow
...
Release v0.7 处于 Phase2，Epic A、Epic B、Epic C Search 2.0 与 Epic D 已完成。Conversation 与 Task 可归属单层 Workspace；Knowledge、Conversation 和选中 Messages 可显式创建保留 SourceRef 快照的 Task。Today 与 Tasks 会展示来源及删除后的降级状态，Search 2.0 可检索并筛选 Task。数据仍保存在 LocalStorage 中，不需要数据库。
...
当前工程统计见 [Project Status](./docs/project-status.md)，本版本变更与限制见 [Release v0.7](./docs/releases/v0.7.md)。
