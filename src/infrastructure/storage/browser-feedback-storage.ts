...
export class BrowserFeedbackStorage {
  ...
  save(input: Pick<Feedback, "category" | "content">) { const item: Feedback = { ...input, id: crypto.randomUUID(), createdAt: new Date().toISOString() }; window.localStorage.setItem(FEEDBACK_KEY, JSON.stringify([item, ...this.getAll()])); return item; }
