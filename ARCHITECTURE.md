...
## Release v0.6 architecture baseline

v0.6 是 Phase2 的 Project Stabilization 文档基线，Epic C 已完成。本次 Release 不改变运行时架构、存储结构或业务行为；系统继续采用 Entity、Contract、BrowserStorage、Service、Page 五类职责，并保持 Demo Provider 默认可用、Ollama 可选启用、人工 Review 必经的边界。
...
Architecture Pack v1 已冻结 Epic D（Task / Productivity Layer）的设计边界，当前只完成 D0 文档，不代表 Task 已实现。Task 将作为独立行动领域：可选关联 Conversation 或 KnowledgeCard、归属单层 Workspace，并拥有自身的日期与完成状态；源删除时 Task 保留并显示 `deleted`，Workspace 删除时 Task 回迁 Inbox。
...
当前运行时的顶层领域关系为 `Workspace → Conversation → Source / Message / ConversationVersion → AnalyzerRun → Proposal → Review → KnowledgeCard → Tag / Search`。Epic D 计划在旁侧增加独立 Task，并在更晚阶段评估 Activity；完整冻结模型见 [docs/architecture/DOMAIN_MODEL.md](./docs/architecture/DOMAIN_MODEL.md)。
...
`Task` 尚未实现。其计划字段、`TaskType`、`SourceRef`、派生视图和删除策略以 RFC-003 为准；在 D1 之前不得把 Task 写入当前 Entity、Contract、Storage 或页面能力清单。`Activity` 同样尚未实现，且不属于 Epic D 第一阶段。
...
Task 计划使用独立 Contract、BrowserStorage 与 Service。Knowledge 不负责行动，Task 不负责知识内容，Provider 不负责业务决策，Search 不拥有数据，未来 Activity 不负责当前业务状态。AI 只能提供待用户确认的 Task 建议，不能直接创建、完成或删除 Task。
